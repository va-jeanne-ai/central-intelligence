...
export function SuggestionPanel({
  ...
  return (
    <div
      className={`bg-accent-50 border border-accent-200 rounded-xl overflow-hidden ${className}`}
    >
      {/* Header */}
      <div className="flex items-center gap-2 px-5 py-3 border-b border-accent-200/60">
        <span className="text-base">{icon}</span>
        <span className="text-sm font-bold text-gray-800">{title}</span>
      </div>

      {/* Items */}
      <div className="divide-y divide-accent-200/40">
        {items.map((item) => (
