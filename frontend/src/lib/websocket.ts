...
type ConnectionState = "disconnected" | "connecting" | "connected" | "closing" | "failed";

// ─── CentralIntelligenceWebSocket ────────────────────────────────────────────────────────

export class CentralIntelligenceWebSocket {
  ...
  constructor(sessionId: string) {
    ...
    const baseUrl = `${wsBase}/central-intelligence/${sessionId}`;
  ...
  connect(): void {
    ...
    try {
      ...
      console.error("[CentralIntelligenceWebSocket] Failed to construct WebSocket:", err);
    ...
    this.ws.onopen = () => {
      if (this.disposed) { this.ws?.close(1000); return; }
    ...
    this.ws.onmessage = (event: MessageEvent<string>) => {
      ...
      try {
        ...
        console.warn("[CentralIntelligenceWebSocket] Could not parse message:", event.data, err);
      }
    };

    this.ws.onerror = (event) => {
      if (this.disposed) return;
      console.error("[CentralIntelligenceWebSocket] WebSocket error:", event);
    };
  ...
  send(message: string): void {
    if (this.ws === null || this.state !== "connected") {
      console.warn("[CentralIntelligenceWebSocket] Cannot send — socket is not connected");
      return;
    }
  ...
  private handleReconnect(): void {
    ...
    if (this.reconnectAttempts >= this.maxReconnects) {
      console.warn(
        `[CentralIntelligenceWebSocket] Max reconnect attempts (${this.maxReconnects}) reached. Giving up.`,
      );
    ...
    console.info(
      `[CentralIntelligenceWebSocket] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.maxReconnects})`,
    );
