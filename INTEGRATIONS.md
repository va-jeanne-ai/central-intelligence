...
| [`/foresight`](frontend/src/app/(app)/foresight/page.tsx) — **statistics engine over `lead_journey` + `email_campaigns` + `insights`/`calls` (P1, 2026-08-07)** | Nightly Celery task (`app.tasks.foresight.compute_foresight_recommendations`, beat-scheduled 04:50 UTC, ~1h after the WGR sync) counts four cohort families — live-vs-replay watch → booking, channel → close rate (`ig_dm`/`meta_paid` vs the all-lead baseline, reusing `bucket_channel_combos`), `email_campaigns.campaign_type='Value/Education'` → open rate (mean-of-campaigns interval, not Wilson), and `insights.signal_family` → discovery-call close rate — and turns each into a Wilson (or mean) 95% confidence interval via the pure `app/services/foresight.py`. A card only **publishes** (as a lift or, when the variant is significantly *below* baseline, a warning) when its interval clears the baseline; overlapping intervals stay **gated**, rendered collapsed with a computed hold reason. Results overwrite `foresight_recommendations` (small, OUR table, full delete+insert) each run; `GET /foresight/recommendations` just reads it. Replaces the P0 static-sample prototype page. |
