...
- **Mirrored free-text columns (UTM values, names, URLs, notes) are `sa.Text`** — the upstream columns are unbounded Postgres `text`, and a `String(n)` mirror would crash the sync on an oversize value (audit r5 #9). Short structured identifiers (WGR/GHL/Meta ids, enum-ish statuses) may stay `String(n)`.
