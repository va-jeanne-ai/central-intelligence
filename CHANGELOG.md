...
**OAuth ("Connect with Meta") — single shared business account.** Replaces the manual token-paste with a proper Facebook Login flow; the manual form stays as a fallback. Modeled on the existing Google per-user OAuth, but stored in the single shared `integrations` row (Instagram is one brand account, not per-staff).

- `app/services/meta_oauth.py` (new) — OAuth primitives: `build_authorize_url` (Facebook dialog, comma-separated scopes), `exchange_code_for_token` (short-lived), `exchange_for_long_lived` / `refresh_long_lived` (~60-day token via `fb_exchange_token` — Meta has no refresh token), `resolve_ig_user_id` (walks `/me/accounts` → Page → `instagram_business_account`), `compute_expiry`, `is_configured`.
- `app/routes/meta_oauth.py` (new) — `GET …/instagram/oauth/start` (auth, Fernet-signed CSRF state + TTL), `GET …/instagram/oauth/callback` (**auth-exempt**: validates state, code→short→long-lived, resolves IG id, writes `{access_token, ig_user_id, expires_at, auth_method:"oauth"}` into the `integrations` row, redirects `?connected=ok|err`), `DELETE …/instagram/oauth/disconnect`. Mounted in `main.py`; callback added to `auth.py` `_EXEMPT_PREFIXES`.
- `app/services/instagram_credentials.py` — `ensure_fresh_token(session, integration)`: re-extends an OAuth long-lived token when within 7 days of expiry (no-op for manual-token rows, which have no `auth_method='oauth'`/`expires_at`). Called at the top of the `social_stats` Instagram sync, so the connection effectively never expires while syncs run. `load_instagram_credentials` unchanged for callers — both connectors write the same `access_token`/`ig_user_id` keys.
- `app/config.py` — `meta_oauth_client_id`/`_secret`/`_redirect_uri` (env `META_OAUTH_*`; empty by default — the connect button errors cleanly until configured).
- `app/services/integrations_registry.py` + `app/schemas/integrations.py` + `app/routes/integrations.py` — `meta_oauth: True` flag on the Instagram provider, surfaced in `ProviderSummary`.
- `frontend/.../integrations/[slug]/page.tsx` — `MetaInstagramConnectCard`: "Connect with Meta" button → `/oauth/start` → `window.location` redirect; handles `?connected=ok|err` (friendly copy for `no_ig_business_account` etc.); shows connected state + Disconnect; manual form remains below as fallback.

**Production gating:** the OAuth scopes (`instagram_manage_insights` etc.) require a Meta app + **App Review** before non-test users can grant them; in development mode an app admin/tester can connect their own account without review. Verified structurally with zero live calls (authorize-URL shape, state round-trip + stale/garbage rejection, `ensure_fresh_token` no-op vs. refresh-window paths, middleware exemption, registry flag). The live consent → token exchange needs your Meta app `client_id`/`secret`.
